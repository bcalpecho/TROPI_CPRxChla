...
##01 to plot annual mean of chlos median ensemble from 2015 to 2100
  ...
  plot_delta_chlos <- function(ensemble){
    ...
    hist_converted <- st_apply(ens_selected,1:3, convert_chlos_to_chla_sqrt, rename = F)  
    
    hist_converted <- hist_converted %>% rename("chla_sqrt" = "chlos")
...
##02 to plot ensemble at 2015 and 2100
  ...
  plot_esm_snapshot <- function(ens){
    ...
    ens_converted <- st_apply(ensemble,1:3, convert_chlos_to_chla_sqrt, rename = F)
    
    ens_converted <- ens_converted %>% rename("chla_sqrt" = "chlos")
...
##04 to compute for summary statistics of chlos
  ...
  summary_stats_chlos <- function(ensemble_list){
    for(i in 1:length(ensemble_list)){
      ...
      # assign("chlos_future_2100_summary", ens_baseline_summary, envir = .GlobalEnv)
      # print("Saved in Global Env: chlos_future_2100_summary")
      ...
      assign("chlos_future_2100_summary", ens_future_summary, envir = .GlobalEnv)
      print("Saved in Global Env: chlos_future_2100_summary")
      ...
      assign("chlos_delta_summary", ens_delta_summary, envir = .GlobalEnv)
      print("Saved in Global Env: chlos_delta_summary")
    }
  }
